...
function __splitTrim(str, token) {
  let originalRes =  str.split(token).map(function (p) {
    return p.trim();
  });
  return originalRes
}
...
function __getMatches(string, regex, index) {
...
function __hierarchize(obj, dottedPath) {
  ...
  if(dottedPath.indexOf("\"") > -1 && DELIMITER == ".") {
    parts = [dottedPath];
  }
  var res = obj;
  for (var p in parts) {
    if (res.hasOwnProperty(parts[p]))
      res = res[parts[p]];
    else if(utils.isExpression(parts[p])){
      try {
        let findColumnsForExpression = /\$(\w*)/g;
        let columnName = /->(\w*)/
        let evalExpression =  parts[p];
        let columnsToReplace = __getMatches(evalExpression,findColumnsForExpression)
        columnsToReplace.forEach(column=>{
          evalExpression = evalExpression.replace('$' + column,obj[column])
        });
        let columnNameMatch = columnName.exec(evalExpression)
        columnNameMatch.forEach(col => {
          evalExpression = evalExpression.replace(`->${col}`,'')
        });
  ...
  else if(Array.isArray(res)) res = res
  ...
  if (result && synopsis.pathway.length > 0) {
    ...
    return result.map(function (ele) {
      ...
      for (var i = 0, len = synopsis.pathway.length; i < len; i++) {
        ...
        //if it's expression take the name
        let colName = __getMatches(key,/->(\w*)/g)
        if(colName && colName.length > 0){
          if(utils.isExpression(key)){
            res[colName[0]] = __hierarchize(ele, key);
          }else{
            res[colName[0]] = __hierarchize(ele, key.replace('->'+colName[0],''));
          }
        }else{
